pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "group1-team6-eureka-client-account"
        IMAGE_TAG = "v1.2"
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'step4', url: 'https://github.com/jghan-no1/eureka-client-account.git'
            }
        }
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/team6-eureka-client-account team6-eureka-client-account=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
